...
async function fetchAllMessages() {
    ...
    try {
        const p1 = axios.get(`https://group-chat-app.apoorvnema.pro/get-all-messages/${groupId}`, { headers: { "Authorization": token } });
        const p2 = axios.get(`https://group-chat-app.apoorvnema.pro/get-all-members/${groupId}`, { headers: { "Authorization": token } });
...
form.addEventListener("submit", async (e) => {
    try {
        ...
        await axios.post(`https://group-chat-app.apoorvnema.pro/send-group-message/${groupId}`, { message }, { headers: { "Authorization": token } });
        document.location.reload();
    }
    catch (err) {
        console.error(err);
...
async function editGroupSettings(navBar) {
    const membersResponse = await axios.get(`https://group-chat-app.apoorvnema.pro/get-all-members/${groupId}`, { headers: { "Authorization": token } });
