...
async function uploadImage(file) {
    try {
        ...
        const response = await axios.post('https://rpi4.apoorvnema.pro/upload-image', formData, {
        ...
        if (admin) {
            ...
            button.onclick = () => {
                ...
                form.addEventListener("submit", async (e) => {
                    ...
                    try {
                        await axios.put(`https://rpi4.apoorvnema.pro/edit-group/${groupId}`, { name: newGroupName, members: newMembers }, { headers: { "Authorization": token } });
                        window.location.href = "/";
                    } catch (err) {
...
async function deleteUser(userId) {
    try {
        await axios.delete(`https://rpi4.apoorvnema.pro/delete-member/${groupId}/${userId}`, { headers: { "Authorization": token } });
...
async function makeAdmin(userId) {
    try {
        await axios.put(`https://rpi4.apoorvnema.pro/make-admin/${groupId}/${userId}`, {}, { headers: { "Authorization": token } });
        document.location.reload();
    } catch (err) {
        alert(err.response.data.error);
    }
...
document.getElementById("delete-group").addEventListener("click", async () => {
    try {
        await axios.delete(`https://rpi4.apoorvnema.pro/delete-group/${groupId}`, { headers: { "Authorization": token } });
        window.location.href = "/";
    } catch (err) {
        alert(err.response.data.error);
    }
