        ...
        if (admin) {
            ...
            button.onclick = () => {
                ...
                form.addEventListener("submit", async (e) => {
                    ...
                    try {
                        await axios.put(`http://192.168.29.184:3000/edit-group/${groupId}`, { name: newGroupName, members: newMembers }, { headers: { "Authorization": token } });
                        window.location.href = "/";
                    } catch (err) {
...
async function deleteUser(userId) {
    try {
        await axios.delete(`http://192.168.29.184:3000/delete-member/${groupId}/${userId}`, { headers: { "Authorization": token } });
...
async function makeAdmin(userId) {
    try {
        await axios.put(`http://192.168.29.184:3000/make-admin/${groupId}/${userId}`, {}, { headers: { "Authorization": token } });
        document.location.reload();
    } catch (err) {
        alert(err.response.data.error);
    }
...
document.getElementById("delete-group").addEventListener("click", async () => {
    try {
        await axios.delete(`http://192.168.29.184:3000/delete-group/${groupId}`, { headers: { "Authorization": token } });
        window.location.href = "/";
    } catch (err) {
        alert(err.response.data.error);
    }
